...
async def queue_task(prompt):
    ...
    try:
        async with asyncio.timeout(10):
            async with http_session.post(f"{HF_API}/generate", json={"prompt": p, "max_length": 80}) as r:
                if r.status != 202:
                    try:
                        j = await r.json()
...
async def wait_for_result(task_id, total_timeout=60):
    if not task_id:
        return "invalid"
...
async def ai_command(interaction, prompt: str):
    ...
    try:
        ...
        if not tid:
            ...
            return
        result = await wait_for_result(tid, total_timeout=60)
        if not result:
...
async def runqueue(interaction):
    ...
    for tid in keys:
        ...
        try:
            ...
            result = await wait_for_result(real_task, total_timeout=60)
            if not result:
                result = "no output"
